...
export const watchRelayUrl = sdk.setupOnInit(async effects => {
  ...
  if (!current) {
    await storeJson.merge(effects, { relayUrl: urls.find(u => u.includes('.local')) }, { allowWriteAfterConst: true })
  } else if (!urls.includes(current)) {
    await storeJson.merge(effects, { relayUrl: urls.find(u => u.includes('.local')) }, { allowWriteAfterConst: true })
    await sdk.action.createOwnTask(effects, setRelayUrl, 'important', {
      reason: i18n('Your relay address changed because the previous one is no longer available'),
    })
  }
})
